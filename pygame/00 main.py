...
def scroll(event):
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        ...
        if event.button == 4 or event.button == 5:
            bubblesort_frame = (spacing, title_height+spacing+scroll_y)
            quicksort_frame = (window_size[0]-spacing-sortingBoxes_size_w, title_height+spacing+scroll_y)
            mergesort_frame = (spacing, title_height+spacing*2+sortingBoxes_size_h+scroll_y)
            insertionsort_frame = (window_size[0]-spacing-sortingBoxes_size_w, title_height+spacing*2+sortingBoxes_size_h+scroll_y)
            bogosort_frame = (spacing, title_height+spacing*3+sortingBoxes_size_h*2+scroll_y)
            radixsort_frame = (window_size[0]-spacing-sortingBoxes_size_w, title_height+spacing*3+sortingBoxes_size_h*2+scroll_y)
            scrollbar_image.set_alpha(200)
...
def cursor(frame, sort_image, show_image):
    global scroll_y, speed, listlength, scroll_y
    ...
    while frame[0]+sortingBoxes_size_w > mousePos[0] > frame[0] and frame[1]+sortingBoxes_size_h > mousePos[1] > frame[1]:
        ...
        while frame[0]+runBtn_x+runBtn_w > mousePos[0] > frame[0]+runBtn_x and frame[1]+runBtn_y+runBtn_h > mousePos[1] > frame[1]+runBtn_y:
            ...
            if not runSelected_drawn:
                window.blit(runSelected_btn, (frame[0]+runBtn_x, frame[1]+runBtn_y-scroll_y))
        ...
        if runSelected_drawn:
            window.blit(runUnselected_btn, (frame[0]+runBtn_x, frame[1]+runBtn_y-scroll_y))
